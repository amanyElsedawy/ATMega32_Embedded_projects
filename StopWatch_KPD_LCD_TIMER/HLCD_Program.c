...
u8 currentLineNb = HLCD_Line1;  // Start on the first line
u8 currentCharNb = 0;  // Start at the first character
...
//set cursor  with global postion
u8 HLCD_GlobalSetCursor(u8 Copy_LineNb, u8 Copy_CharNb) {
    u8 Local_u8ErrorState = STD_TYPE_OK;
    ...
    if (Copy_CharNb < 40) {
        ...
        currentLineNb = Copy_LineNb;
        currentCharNb = Copy_CharNb;
    } else {
        Local_u8ErrorState = STD_TYPE_NOK;
    }
...
void moveCursorLeft() {
    if (currentCharNb > 0) {
        currentCharNb--;
        HLCD_GlobalSetCursor(currentLineNb, currentCharNb);
    }
}
